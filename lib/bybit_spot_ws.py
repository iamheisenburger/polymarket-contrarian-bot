"""
Bybit Spot WebSocket v5 price feed. Same interface as CoinbasePriceFeed for
drop-in use in MultiPriceFeed aggregation.

Supports: BTC, ETH, SOL, XRP, DOGE, BNB (all as USDT pairs on Bybit spot).
HYPE may not be listed on Bybit — filtered out if not in SYMBOL_MAP.
"""
import asyncio
import json
...
BYBIT_SPOT_SYMBOLS = {
    ...
    "DOGE": "DOGEUSDT",
    "BNB": "BNBUSDT",
    # HYPE not typically on Bybit spot as of 2026 — omitted defensively.
}
SYMBOL_TO_COIN = {v: k for k, v in BYBIT_SPOT_SYMBOLS.items()}
...
class BybitSpotPriceFeed:
    """Real-time spot price feed from Bybit WS v5. Same interface as
    ...
    def __init__(
        ...
        self.coins = [c for c in requested if c in BYBIT_SPOT_SYMBOLS]
        skipped = [c for c in requested if c not in BYBIT_SPOT_SYMBOLS]
        if skipped:
            logger.info(f"BybitSpotPriceFeed: skipping unsupported coins: {skipped}")
        ...
        self._state: Dict[str, CoinState] = {
            c: CoinState(symbol=BYBIT_SPOT_SYMBOLS[c]) for c in self.coins
        }
    ...
    async def start(self) -> bool:
        ...
        for _ in range(50):
            ...
            if all(self._state[c].price > 0 for c in self.coins):
                return True
        logger.warning("BybitSpotPriceFeed: timeout waiting for initial prices")
    ...
    async def _ping_loop(self, ws):
        """Bybit requires a ping every 20s to keep the WS alive."""
        try:
            while self._running:
    ...
    async def _run_loop(self):
        ...
        while self._running:
            try:
                logger.info(f"BybitSpotPriceFeed connecting: {self.coins}")
                async with ws_connect(BYBIT_SPOT_WS_URL) as ws:
                    self._ws = ws
                    args = [f"tickers.{BYBIT_SPOT_SYMBOLS[c]}" for c in self.coins]
                    sub_msg = json.dumps({"op": "subscribe", "args": args})
                    await ws.send(sub_msg)
                    logger.info("BybitSpotPriceFeed connected + subscribed")
                ...
                logger.warning(f"BybitSpotPriceFeed error: {e}, reconnecting in 2s")
                self._ws = None
                if self._ping_task:
                    self._ping_task.cancel()
                    self._ping_task = None
    ...
    def _handle_message(self, raw: str):
        try:
            ...
            # Skip non-ticker control messages (op responses, pongs).
            ...
            if not payload:
                return
            # Ticker payload can be dict (snapshot) or nested. Handle both.
            ...
            for entry in entries:
                ...
                if not coin or coin not in self._state:
                    continue
                last_str = entry.get("lastPrice") or entry.get("last") or entry.get("close")
                if last_str is None:
                    continue
